...
#else
	    /* BV: never mind about authoritative answers these days */
	    /* but really should allow configuring a forwarder */
	    ercode = UE_NAM;
#endif
...
#error Not finished
    ...
    if(q->state == done) {
#if 1
	/* BV: get a new port for the next query */
	if (!udp_init(-1,0))
	  bughlt("resolv","couldn't initialize UDP package again");
#endif
	i = q->quid;
	dequeue(q);
